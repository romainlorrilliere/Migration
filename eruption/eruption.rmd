...
```{r package}
vecPackage=c("move","lubridate","ggplot2","ggmap","sf","ggspatial","data.table","maps")#,"moveVis",suncalc","HelpersMG","adehabitatHR","rtide")
...
gg <- ggplot(data  = d[species =="crow",], aes(x = timestamp,y = heading, colour=name,group=name))+ facet_grid(species~.)+ theme(legend.position="none")
gg <- gg + geom_rect(xmin=p1s,xmax=p1e,ymin = -Inf, ymax = Inf,color = NA,fill="#df6969",alpha=0.1)
gg <- gg + geom_rect(xmin=p2s,xmax=p2e,ymin = -Inf, ymax = Inf,color = NA,fill="#6e69df",alpha=0.1)
gg <- gg + geom_rect(xmin=p3s,xmax=p3e,ymin = -Inf, ymax = Inf,color = NA,fill="#df6969",alpha=0.1)
gg
gg <- gg + geom_line() + geom_point(size=1)
gg


gg <- ggplot(data  = d[species =="crow",], aes(x = timestamp,y = ground_speed, colour=name,group=name))+ facet_grid(species~.)+ theme(legend.position="none")
gg <- gg + geom_line() + geom_point(size=1)
gg
